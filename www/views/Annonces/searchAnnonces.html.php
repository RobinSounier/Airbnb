...
<div class="min-h-screen bg-gray-50 pb-12">
    <div class="bg-white border-b border-gray-200 py-6 mb-8 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-extrabold text-gray-900">Resultat de votre recherche</h1>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                <?php foreach ($rooms as $room): ?>
                    ...
                    <div class="group block w-full rounded-xl cursor-pointer hover:shadow-xl transition-shadow duration-300 bg-white overflow-hidden border border-gray-100">
                        ...
                        <a href="/room/<?= htmlspecialchars($id) ?>" class="block"
                           title="<?= htmlspecialchars($title) ?>">
                            ...
                            <div class="aspect-w-16 aspect-h-9 relative h-64 bg-gray-200">
                                ...
                                <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-bold shadow-sm text-gray-900">
                                    <?= htmlspecialchars($price) ?> € <span
                                            class="font-normal text-xs text-gray-500">/ nuit</span>
                                </div>
                                <div class="absolute top-12 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-bold shadow-sm text-gray-900">
                                    <?= htmlspecialchars($number_of_bed) ?> <span
                                            class="font-normal text-xs text-gray-500"> lits</span>
                                </div>
                            </div>
                            ...
                            <div class="p-4 pb-2">
                                ...
                                <p class="text-gray-500 text-sm mb-4 line-clamp-2"><?= htmlspecialchars($city) ?>
                                    , <?= htmlspecialchars($country) ?></p>
                            </div>
                        </a>
                        <div class="px-4 pb-4 pt-0">
                            <a href="/reservation/create?room_id=<?= $id ?>"
                               class="block w-full py-2.5 text-center text-white font-medium bg-[#FF5A5F] hover:bg-[#E14C50] rounded-lg transition-colors shadow-md hover:shadow-lg">
                                Réserver
                            </a>
                        </div>
        ...
        <?php else: ?>
            <div class="text-center py-24 bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <h3 class="text-lg font-medium text-gray-900">Aucun resultat pour votre recherche</h3>
